...
// Adapted for this desktop bridge: removed Android dependencies; validated bone parents and vertex blocks.
// Modified for the desktop compatibility adapter: Android dependencies removed;
// local decoding interfaces and structural validation adapted. See THIRD_PARTY_NOTICES.md.
...
class Loader {
	...
	static Model loadMbacData(byte[] bytes) throws IOException {
		...
		if (available > 0) {
			System.err.println("Uninterpreted bytes in MBAC (" + available + ", v=" + version);
		}
	...
	static Action[] loadMtraData(byte[] bytes) throws IOException {
		...
		if (transTypeCounts[7] != 0) {
			...
			System.err.println("ActTableData: transTypeCounts[7] = " + transTypeCounts[7]);
		}
		//noinspection unused
		...
		for (int action = 0; action < numActions; action++) {
			...
			final TreeMap<Integer,Integer> sparseIntArray = new TreeMap<>();
		...
		if (available > 0) {
			System.err.println("ActTableData: uninterpreted bytes in MTRA");
		}
	...
	private void readPolyC(Model data, int numVertex, int numColor, int numTriangles) throws IOException {
		...
		if (unknownByte != 0) System.err.println("PolyC unknownByte = " + unknownByte);
	...
	private void readPolyV3(Model data, int numVertex, int numTriangles) throws IOException {
		...
		if (unknownByte != 0) System.err.println("PolyT v3: unknownByte = " + unknownByte);
	...
	private int readUBits(int size) throws IOException {
		if (size > 25) {
			System.err.println("readUBits(size=" + size + ')');
