...
public final class AbyssalImporter extends GodotPlugin {
    private static final int PICK = 4821, CONVERT = 4822, SAVE_PICK = 4823, SAVE_WRITE = 4824, IMAGE_PICK = 4825, FILE_WRITE = 4826;
    ...
    /** A file of the game's own waiting to be copied out to a chosen document. */
    ...
    @Override public Set<SignalInfo> getPluginSignals() {
        return new HashSet<>(Arrays.asList(new SignalInfo("progress", String.class),
            ...
            new SignalInfo("file_exported", String.class)));
    }
    ...
    /** Copies one of the game's own files out to a document the player names. */
    @UsedByGodot public void export_file(String name, String mime, String path) {
        pendingFile = path;
        getActivity().runOnUiThread(() -> {
            ...
            try { getActivity().startActivityForResult(intent, FILE_WRITE); }
            catch (Exception e) { pendingFile = null; emitSignal("image_failed", "No file picker is available: " + e.getMessage()); }
        });
    }
    ...
    private void deliverFile(Uri uri) {
        final String path = pendingFile;
        pendingFile = null;
        if (path == null) { emitSignal("image_failed", "The file was no longer ready."); return; }
        new Thread(() -> {
            try (InputStream src = new FileInputStream(path);
                 OutputStream dst = getActivity().getContentResolver().openOutputStream(uri, "wt")) {
                if (dst == null) throw new IOException("The chosen location refused the file.");
                byte[] buffer = new byte[65536]; int count;
                while ((count = src.read(buffer)) != -1) dst.write(buffer, 0, count);
                dst.flush();
                getActivity().runOnUiThread(() -> emitSignal("file_exported", "Copied out."));
            } catch (Exception e) {
                final String reason = String.valueOf(e.getMessage());
                getActivity().runOnUiThread(() -> emitSignal("image_failed", "Could not write the copy: " + reason));
            }
        }, "abyssal-file-export").start();
    }
    ...
    @Override public void onMainActivityResult(int request, int result, Intent data) {
        ...
        if (request == FILE_WRITE) {
            if (result == Activity.RESULT_OK && data != null && data.getData() != null) deliverFile(data.getData());
