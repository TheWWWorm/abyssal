...
func run() -> void:
 ...
 expect(app.modal.visible and root.gui_get_focus_owner()!=null and root.gui_get_focus_owner().text=="Back","A opens Help with its Back button focused")
 await tap(JOY_BUTTON_A);await settle()
 expect(not app.modal.visible and focused_text()=="Help","Back closes Help and returns to the entry that opened it")
