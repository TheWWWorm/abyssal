...
| Camera / lights | C / L | D-pad left / right | Pause menu / Settings → Graphics |
...
**Settings** is one screen, the same from the main menu, the pause menu and a station's System menu, with five tabs: **Audio**, **Graphics**, **Display**, **Controls** and **Gameplay**. LB / RB switch tabs. **Controls** opens **Steering**, **Gamepad**, **Touch controls**, **Key bindings** and **Control reference**; leaving one puts the highlight back on the row that opened it. Between them they offer keyboard remapping, mouse sensitivity and inversion, tilt steering, controller deadzone and inversion, touch look sensitivity, Touch **Auto / On / Off**, touch control placement, and left/right handling as **Auto / Always strafe / Always turn**. **Gameplay tips & control hints** (Settings → Gameplay) hides loading tips, one-time M.A.I. guidance and the flight control reminder; objectives and action prompts remain visible. Only the most recently used controller owns flight. The game pauses if a controller disconnects or the window loses focus. In the browser it also pauses if the page gives your pointer back, which is what pressing Escape there does.
...
**Steer by tilting**, under Controls · Steering, flies the submarine by tilting a phone or tablet. It reads the device's own motion sensor, so a desktop machine has nothing to offer it, and a browser asks permission the moment you switch it on. Tilt turns; it never strafes, and the stick and touch controls keep working alongside it. **Calibrate tilt** makes your current grip the neutral position, and **Tilt sensitivity** sets how far you have to lean for a full turn. The browser reads tilt in the visible screen's orientation and recentres after the display rotates or the sensor pauses. (The Android build before 1.2.1 never asked the phone for its sensors, so tilt reported no reading there.)
...
In menus, use the D-pad or left stick to move, **A** to select and **B** to go back. Dropdowns use the same controls; left/right adjusts sliders. In Touch **Auto**, using a keyboard, mouse or gamepad hides the flight pads until the screen is touched again. Controller drift and browser mouse events generated by a touch gesture do not switch the controls, including on iPhone Safari. **On** and **Off** remain manual overrides.

A and D, and the gamepad left stick, slide the submarine sideways rather than turning it. Turning is the mouse, or the right stick. On a touchscreen the thumb stick keeps turning instead, because a thumb has nothing else to turn with. **Left/right keys and stick** in Settings → Controls → Steering overrides that choice on any platform. Lateral thrust runs at a fraction of cruise speed and ignores the throttle, so a stopped submarine can still slide out of trouble, and it counts as manual control: strafing disengages an autopilot course.

On touch controls, the submarine levels while steering and the chase camera keeps the water upright during ordinary turns. Diagonal thumb movement can still pitch and yaw together without rolling the entire view.
...
The station menus are glass panels over the docked station: the six services as tiles, the hangar, missions and status as cards with the game's own pictures, and the shops as a list beside a detail panel with the item's figures and the purchase. The ship dealer compares your hull with the one for sale, and the equipment shop's ship line opens the ship and cargo page. A destination outside the hull's safe depth is not refused: as in the original, the chart warns that it lies outside the safety zone and asks whether to travel there.

On a touchscreen the stick appears wherever your left thumb lands and returns to its resting corner when you lift off; **Settings → Controls → Touch controls → Steering stick → Fixed in place** keeps it where it is placed instead. Dragging any free area of the screen swings the camera round the submarine without steering it. The camera stays where you left it until you touch the stick again or a few seconds pass. **Invert vertical steering** applies to the stick, the drag and the mouse. On a widescreen phone the interface keeps a margin on both sides, clear of the notch or camera cutout whichever way the phone is turned; held upright it keeps clear of the notch and status bar at the top and the home indicator at the bottom. The view behind it still fills the screen. Held upright, controls and text keep the size they have in landscape, the harpoon sits above the guns and the booster above the stick, and the camera widens its view (about 50° across instead of 32° on a 19.5:9 phone) rather than showing a narrow slit of what is ahead. The touch HUD shows hull and armour, shield, cargo, credits and station in the top left, pause and map in the top right, and a short depth gauge covering the hull's safe band. Armour is drawn in yellow while it lasts; once it is gone the same bar shows the hull in green. The throttle is an arc round the guns button: slide along it to set the speed. Buttons for a harpoon, guns or booster appear only when one is fitted, and a ship without guns has its harpoon in the guns' place. The booster button refills from the bottom while it recharges. DOCK appears, pulsing, only when there is a station or gate to dock with. The camera view and fullscreen are in the pause menu. On a desktop the same gauges are used: condition, cargo, credits and station in the top left and the depth gauge on the right, whose ends take the colours of the depth-limit panels (gold above the safe band, deep blue below it), with thrust, time and the booster on a strip at the foot of the screen; the booster half appears only when one is fitted. iPhone browsers have no fullscreen control: open the game in Safari, use Share → Add to Home Screen and start it from that icon to play without browser bars (other iOS browsers cannot add it). Where a browser does offer fullscreen, the button in the top-right corner of the main menu and pause menu, and the pause menu entry, switch it; the entry is dropped once the game already runs without browser bars.
...
On the world map, tap a station, drag to pan and pinch to zoom. With a mouse, right-drag and scroll. With a controller the left stick moves the zone, LB / RB step through the stations in the side view, LT / RT zoom, and the D-pad moves between the buttons; the map opens on **Set station autopilot**. The station list is searchable too.
...
Map search offers continuous autopilot or a STREAM transfer. Time acceleration runs ordinary simulation ticks - T cycles 1×/2× in manual flight and up to 16× during clear autopilot travel, dropping back to 1× near danger or on arrival. Autopilot passes stations vertically within your ship's pressure limits, steers around their modules on approach, and docks when it reaches a clear berth. An active encounter must still be completed before docking.
...
Markets are stocked and priced as the original stocks them: up to four lines of goods that come more readily the nearer their home, two to five pieces of equipment gated by rarity and chapter, up to three hulls, the special holdings that sell everything, and the imports that come in between visits; goods are worth more the further they are from home. In the equipment store, **Shop** lists systems for sale and **Ship Equipment** lists installed systems. Both show the current and maximum equipment slots and cargo capacity; **Ship and Cargo** shows those limits too. The job board posts one to eight offers of the kinds each side gives, fees and deposits on the fifty, each with the client's face, the destination's depth and distance and a difficulty. Colonists do not trade: they take whatever is in your hold at its floor price when you dock, and pay a bounty on the pirates you have sunk that grows with the square of their number. Rebel holdings keep your cargo for trading and making goods. The tutorial chapters keep the original's locks, the job board opens at the ninth chapter and the chart at the seventh, and a complete medal collection buys the Aquarius for 50,000. Every random draw comes from the game's own generator, in the original's order.

The phone game's Help is here: **Help** in the title, pause and System menus opens its Instructions, ten topics of the imported text, its Controls and its Credits. The loading screen carries one of its Tips & Tricks. M.A.I. says her one-time pieces the first time their moment comes - the gate, the dock, radiation and pressure, the booster she wants, the catch that got away, the hold that is full - and remembers them in the save, so loading or updating does not start them again. **Gameplay tips & control hints** under Settings → Gameplay turns all of those optional tips off. The station cards of the collection are still shown when they are earned. There are three save slots and the autosave, chosen on Load and on Save game at a station. A hull hit buzzes a pad or a handheld (Controls · Gamepad · Vibration). A hull left on its back rolls itself upright once the helm is released, as the phone game's does, and the water outside a berth keeps moving while you are at the station. Every menu page has one way back: the BACK in its corner, Esc, or B on a pad.
...
This is **release 1.7.0**. Its provenance is not a clean-room one, as this section explains, and that question is unresolved.
