...
The water is the phone game's water. Twenty creatures of the local habitat swim around you wherever you go - anything the camera leaves four hundred metres behind is set down three hundred metres out again, so there is always something to catch - the station's own ships go their rounds past the gates, and from the twelfth chapter on there is a growing chance of pirates waiting ahead. Each story chapter fields the encounter the original stages for it: the minefield of chapter 23, the five freighters of 29, the boss fights, the finale with its own camera shots; contracts are laid out by their kind with the original's numbers. Ships fight as the original's do, harpooned creatures run and fight with their own strength and are lost the moment you look away, wrecks sink and can be hooked for salvage, mines arm within thirty metres and go off two seconds later, capsules fall through the water and are dropped in again.
...
**Textures, from the title menu.** **Mods → Textures** on the title screen lists the two atlases the game is drawn from, each in its opaque form and its cut-out form (the polygons the original saw through), over a checkerboard so transparency shows, with what stands for each now and **View**, **Replace…** (a PNG from your device, through the system file picker - on Android too) and **Restore**; on desktop it also opens the mods folder and the folder of the imported originals. A replacement applies at once. A cut-out replacement (`deep.alpha.png`) is optional: without one, the opaque replacement's own alpha channel serves. The skybox atlas is not offered because this engine does not draw it - the water is its own sky, and only one texel of the original's skybox tints the shallows. Everything below is the same thing done by hand.

**Textures.** Put a PNG at `mods/textures/deep.png`, `mods/textures/fx.png` or `mods/textures/skybox.png` to replace that atlas. `deep` carries the submarines, creatures and stations, `fx` the shots, explosions and effects, `skybox` the surface. Any size works: a 4x or 8x repaint is drawn at more pixels per texel, but keep the original's layout, because every model addresses the atlas by the original's texel positions. The PNG's own alpha channel is used where the original used cut-outs. The originals are in your content cache (`user data folder/content/<hash>/data/textures/`, or **Open original textures** on the Mods page), three files per atlas: `deep.bmp` is the raw palette bitmap from the JAR, `deep.bmp.png` the same pixels as an opaque PNG - the one to paint over - and `deep.bmp.alpha.png` the same again with the phone's transparent palette colour cut out, which the engine draws on the polygons the original flagged as see-through. Your one `deep.png` serves both: keep it opaque except where `deep.bmp.alpha.png` is transparent.
